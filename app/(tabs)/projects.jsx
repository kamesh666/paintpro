...
import { useProjects, useUpsertProject, useClients } from '../../hooks/useSupabase';
import { useAuthStore } from '../../store/authStore';
import { Colors, Spacing, FontSize } from '../../constants/colors';
import { formatCurrency, formatDate, PROJECT_STATUS_LABELS } from '../../lib/utils';

const INPUT = { backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 12, fontSize: 15, color: '#1A1A2E' };
...
function StatusBadge({ status }) {
  ...
  return (
    <View style={[styles.badge, { backgroundColor: s.bg }]}>
      ...
      <Text style={[styles.badgeText, { color: s.text }]}>{PROJECT_STATUS_LABELS[status] ?? status}</Text>
    </View>
  );
}

function ProjectCard({ project, onPress, onPhotos, onMaterials, onPayments, onQuotation }) {
  return (
...
function ProjectModal({ visible, onClose, project }) {
  ...
  // ✅ KEY FIX: individual state + useEffect reset
      ...
      <View style={styles.modalContainer}>
        ...
        <ScrollView style={styles.modalBody} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <TextInput style={INPUT} value={title} onChangeText={setTitle}
            placeholder="e.g. 3BHK interior painting" placeholderTextColor="#9CA3AF"
          ...
          <TextInput style={INPUT} value={location} onChangeText={setLocation}
            placeholder="e.g. Tiruppur" placeholderTextColor="#9CA3AF" underlineColorAndroid="transparent" />
          ...
          <TextInput style={INPUT} value={startDate} onChangeText={setStartDate}
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            <View style={{ flex: 1 }}>
              ...
              <TextInput style={INPUT} value={totalValue} onChangeText={setTotalValue}
                placeholder="0" placeholderTextColor="#9CA3AF" keyboardType="numeric" underlineColorAndroid="transparent" />
            </View>
            <View style={{ flex: 1 }}>
...
const FILTERS = [
  { key: null, label: 'All' }, { key: 'active', label: 'Active' },
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchWrap}>
        ...
        <TextInput style={styles.searchInput} value={search} onChangeText={setSearch}
          placeholder="Search projects..." placeholderTextColor="#9CA3AF" underlineColorAndroid="transparent" />
      ...
      {isLoading ? (
        ...
        <FlatList data={filtered} keyExtractor={item => item.id}
          renderItem={({ item }) => <ProjectCard project={item} onPress={() => openEdit(item)} onPhotos={() => router.push({ pathname: '/site-photos', params: { projectId: item.id, projectTitle: item.title } })}
              onMaterials={() => router.push({ pathname: '/material-costs', params: { projectId: item.id, projectTitle: item.title } })}
              onPayments={() => router.push({ pathname: '/client-payments', params: { projectId: item.id, projectTitle: item.title } })}
              onQuotation={() => router.push({ pathname: '/quotation', params: { projectId: item.id, projectTitle: item.title } })} />}
...
const styles = StyleSheet.create({
  ...
  cardActions: { flexDirection: 'row', gap: 6, marginTop: 8, paddingTop: 8, borderTopWidth: 0.5, borderTopColor: '#F0F0F0' },
  actionBtn:   { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, backgroundColor: Colors.background, borderWidth: 0.5, borderColor: '#E0E0E0' },
