...
import {
  useWorkers, useUpsertWorker,
  useLabourLogs, useAddLabourLog, useMarkLabourPaid,
  useProjects,
...
function SkillBadge({ skill }) {
  ...
  return (
    <View style={[styles.badge, { backgroundColor: s.bg }]}>
      ...
      <Text style={[styles.badgeText, { color: s.text }]}>{SKILL_TYPE_LABELS[skill] ?? skill}</Text>
    </View>
  );
}
...
function WorkerModal({ visible, onClose, worker }) {
  ...
  const handleSave = async () => {
    if (!name.trim())  { Alert.alert('Required', 'Worker name is required'); return; }
    if (!dailyRate)    { Alert.alert('Required', 'Daily rate is required');  return; }
    setSaving(true);
    try {
      ...
      <View style={styles.modalContainer}>
        ...
        <ScrollView style={styles.modalBody} keyboardShouldPersistTaps="handled">
          ...
          <TextInput style={INPUT} value={name} onChangeText={setName}
            placeholder="e.g. Ravi Kumar" placeholderTextColor="#9CA3AF"
          ...
          <TouchableOpacity style={styles.picker} onPress={() => setSkillOpen(!skillOpen)}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <View style={[styles.dot, { backgroundColor: SKILL_COLORS[skillType]?.dot }]} />
              <Text style={styles.pickerValue}>{SKILL_TYPE_LABELS[skillType]}</Text>
            </View>
            ...
            <View style={styles.dropdown}>
              {Object.entries(SKILL_TYPE_LABELS).map(([key, label]) => (
                <TouchableOpacity key={key}
                  style={[styles.dropItem, skillType === key && styles.dropItemActive]}
                  onPress={() => { setSkillType(key); setSkillOpen(false); }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <View style={[styles.dot, { backgroundColor: SKILL_COLORS[key]?.dot }]} />
                    <Text style={[styles.dropText, skillType === key && { color: Colors.primary, fontWeight: '700' }]}>{label}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
...
function LogModal({ visible, onClose }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Error', e.message.includes('unique')
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={styles.modalContainer}>
        ...
        <ScrollView style={styles.modalBody} keyboardShouldPersistTaps="handled">
          ...
          <TouchableOpacity style={styles.picker} onPress={() => { setProjectOpen(!projectOpen); setWorkerOpen(false); }}>
            ...
            <View style={styles.dropdown}>
              {(projects ?? []).filter(p => p.status === 'active').map(p => (
                <TouchableOpacity key={p.id}
                  style={[styles.dropItem, projectId === p.id && styles.dropItemActive]}
          ...
          <TouchableOpacity style={styles.picker} onPress={() => { setWorkerOpen(!workerOpen); setProjectOpen(false); }}>
            <Text style={selectedWorker ? styles.pickerValue : styles.pickerPlaceholder}>
              {selectedWorker ? `${selectedWorker.name} · ₹${selectedWorker.daily_rate}/day` : 'Select worker'}
            </Text>
            ...
            <View style={styles.dropdown}>
              {(workers ?? []).map(w => (
                <TouchableOpacity key={w.id}
                  ...
                  <Text style={styles.dropSub}>{SKILL_TYPE_LABELS[w.skill_type]} · ₹{w.daily_rate}/day</Text>
                </TouchableOpacity>
              ))}
          ...
          <TextInput style={INPUT} value={logDate} onChangeText={setLogDate}
          ...
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            {[
              { val: '0',   label: 'Absent', bg: '#F3F4F6', tc: '#9CA3AF', abc: '#E5E7EB' },
              { val: '0.5', label: '½ day',  bg: '#FEF3C7', tc: '#92400E', abc: '#FDE68A' },
              { val: '1',   label: '1 shift', bg: '#DBEAFE', tc: '#1E40AF', abc: '#BFDBFE' },
              { val: '1.5', label: '1½ shift',bg: '#D1FAE5', tc: '#065F46', abc: '#A7F3D0' },
            ].map(opt => (
              <TouchableOpacity key={opt.val}
                style={[
                  styles.dayChip,
                  daysWorked === opt.val && { backgroundColor: opt.bg, borderColor: opt.abc },
                ]}
                onPress={() => setDaysWorked(opt.val)}>
                <Text style={[
                  styles.dayChipText,
                  daysWorked === opt.val && { color: opt.tc, fontWeight: '800' },
                ]}>{opt.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {amount > 0 && (
            ...
            Amount to pay ({daysWorked === '0' ? 'Absent' : daysWorked === '0.5' ? 'Half day' : daysWorked === '1' ? '1 shift' : daysWorked === '1.5' ? '1½ shifts' : daysWorked + ' shifts'})
          </Text>
...
export default function LabourScreen() {
  ...
  const { data: workers,  isLoading: wLoading, refetch: wRefetch, isRefetching: wRefetching } = useWorkers();
  const { data: projects } = useProjects('active');
  const { data: logs, isLoading: lLoading, refetch: lRefetch, isRefetching: lRefetching } = useLabourLogs(selectedProject);
  const markPaid = useMarkLabourPaid();

  const handleMarkPaid = (logId) => {
    Alert.alert('Mark as paid', 'Confirm payment?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Confirm', onPress: () => markPaid.mutate({ id: logId, projectId: selectedProject }) },
    ]);
  };

  const totalDue  = (logs ?? []).filter(l => !l.is_paid).reduce((s, l) => s + Number(l.amount ?? 0), 0);
  const totalPaid = (logs ?? []).filter(l =>  l.is_paid).reduce((s, l) => s + Number(l.amount ?? 0), 0);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Tab switcher */}
      <View style={{ flexDirection: 'row', alignItems: 'center', margin: 16, marginBottom: 8, gap: 8 }}>
        <View style={[styles.tabRow, { flex: 1, margin: 0 }]}>
          {[{ key: 'logs', label: '📋 Logs' }, { key: 'workers', label: '👷 Workers' }].map(t => (
        ...
        <TouchableOpacity
          style={{ backgroundColor: Colors.accent, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 10 }}
          onPress={() => router.push('/weekly-labour')}
          activeOpacity={0.85}>
          <Text style={{ color: '#fff', fontWeight: '700', fontSize: 12 }}>📆 Weekly</Text>
        </TouchableOpacity>
      </View>
      ...
      {tab === 'logs' && (
        <View style={{ flex: 1 }}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}
            ...
            <TouchableOpacity style={[styles.chip, !selectedProject && styles.chipActive]} onPress={() => setSelectedProject(null)}>
              <Text style={[styles.chipText, !selectedProject && styles.chipTextActive]}>All projects</Text>
            </TouchableOpacity>
            {(projects ?? []).map(p => (
              <TouchableOpacity key={p.id}
                style={[styles.chip, selectedProject === p.id && styles.chipActive]}
                onPress={() => setSelectedProject(p.id)}>
                <Text style={[styles.chipText, selectedProject === p.id && styles.chipTextActive]} numberOfLines={1}>{p.title}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          ...
          {(logs ?? []).length > 0 && (
            <View style={styles.summaryRow}>
              <View style={styles.summaryItem}>
                ...
                <Text style={[styles.summaryValue, { color: Colors.danger }]}>{formatCurrency(totalDue)}</Text>
              </View>
              ...
              <View style={styles.summaryItem}>
                ...
                <Text style={[styles.summaryValue, { color: Colors.primary }]}>{formatCurrency(totalDue + totalPaid)}</Text>
              </View>
            </View>
          )}
          ...
          {lLoading ? (
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={[styles.logCard, item.is_paid && { opacity: 0.65 }]}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.logWorker}>{item.workers?.name ?? 'Unknown'}</Text>
                    <Text style={styles.logMeta}>{formatDate(item.log_date)} · {item.days_worked} day{item.days_worked !== 1 ? 's' : ''}</Text>
                  </View>
              ...
              refreshControl={<RefreshControl refreshing={lRefetching} onRefresh={lRefetch} tintColor={Colors.primary} />}
            />
          )}
        </View>
      )}
        ...
        <View style={{ flex: 1 }}>
          {wLoading ? (
            ...
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyMsg}>Tap + to add your first worker</Text>
            </View>
          ) : (
            <FlatList
              data={workers}
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.workerCard}
                  onPress={() => { setSelectedWorker(item); setWorkerModalOpen(true); }}
                  activeOpacity={0.8}>
                  <View style={styles.workerAvatar}>
                    <Text style={styles.workerAvatarText}>{item.name?.[0]?.toUpperCase() ?? '?'}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.workerName}>{item.name}</Text>
                    <Text style={styles.workerRate}>{formatCurrency(item.daily_rate)}/day</Text>
                  </View>
              ...
              contentContainerStyle={{ padding: 16, paddingTop: 0, paddingBottom: 100 }}
              showsVerticalScrollIndicator={false}
              refreshControl={<RefreshControl refreshing={wRefetching} onRefresh={wRefetch} tintColor={Colors.primary} />}
            />
          )}
        </View>
      )}
      ...
      <TouchableOpacity style={styles.fab}
        onPress={() => tab === 'logs' ? setLogModalOpen(true) : setWorkerModalOpen(true)}
        activeOpacity={0.85}>
      ...
      <WorkerModal
        ...
        onClose={() => { setWorkerModalOpen(false); setSelectedWorker(null); }}
...
const styles = StyleSheet.create({
  ...
  tabRow:      { flexDirection: 'row', margin: 16, marginBottom: 8, backgroundColor: '#FFFFFF', borderRadius: 12, padding: 4, borderWidth: 1, borderColor: '#E0E0E0' },
  tabBtn:      { flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 8, borderRadius: 10 },
  ...
  logCard:     { backgroundColor: '#FFFFFF', borderRadius: 12, marginBottom: 10, padding: 14, flexDirection: 'row', alignItems: 'center', elevation: 1, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 6 },
  logWorker:   { fontSize: 15, fontWeight: '700', color: '#1A1A2E', marginBottom: 2 },
  logMeta:     { fontSize: 12, color: Colors.textMuted },
  logAmount:   { fontSize: 15, fontWeight: '800', color: '#1A1A2E' },
  paidBadge:   { backgroundColor: '#E1F5EE', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10 },
  paidText:    { fontSize: 11, color: Colors.success, fontWeight: '700' },
  payBtn:      { backgroundColor: Colors.primary + '18', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6 },
  payBtnText:  { fontSize: 12, color: Colors.primary, fontWeight: '700' },
  workerCard:  { backgroundColor: '#FFFFFF', borderRadius: 12, marginBottom: 10, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 12, elevation: 1 },
  workerAvatar:{ width: 44, height: 44, borderRadius: 22, backgroundColor: Colors.primary + '18', alignItems: 'center', justifyContent: 'center' },
  workerAvatarText: { fontSize: 17, fontWeight: '700', color: Colors.primary },
  workerName:  { fontSize: 15, fontWeight: '700', color: '#1A1A2E', marginBottom: 2 },
  workerRate:  { fontSize: 13, color: Colors.textSecondary },
  badge:       { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 20 },
  dot:         { width: 6, height: 6, borderRadius: 3 },
  badgeText:   { fontSize: 11, fontWeight: '600' },
  dayChip:     { width: 48, height: 44, borderRadius: 10, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#D1D5DB', alignItems: 'center', justifyContent: 'center' },
  dayChipActive:{ backgroundColor: Colors.primary, borderColor: Colors.primary },
  dayChipText: { fontSize: 13, fontWeight: '600', color: Colors.textSecondary },
  amountPreview: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: Colors.primary + '10', borderRadius: 10, padding: 14, marginTop: 12, borderWidth: 1, borderColor: Colors.primary + '30' },
  amountLabel: { fontSize: 13, color: Colors.primary, fontWeight: '600' },
  amountValue: { fontSize: 20, fontWeight: '800', color: Colors.primary },
  skeleton:    { height: 72, backgroundColor: '#E0E0E0', borderRadius: 12, opacity: 0.5 },
